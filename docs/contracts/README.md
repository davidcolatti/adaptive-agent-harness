...
| `storage.md` | active (M2-T5, M2-T6, M2-T7; extended by M5-T1 and M3-T3) | The `Storage` port the harness persists through, the outcome ledger row it writes, the thirteen-table Supabase schema behind it, the workflow-registry and decision-evidence methods later milestones added, and the sink that drains a run's trace into it. |
...
| `workflow-registry.md` | active (M5-T1 through M5-T7) | The workflow registry: the seven statuses and the transition table between them, what a version declares about the jobs it can handle, the exact-match compatibility selector with its nine ordered checks and typed rejection reasons, the three records and their parse boundaries, the six `Storage` methods and the promotion ledger. Also the **router**, which is an `AgentRuntime` that chooses between a compiled workflow and the full agent, the fallback handoff and the trace span that links the two, and the rolling-window circuit breaker. |
| `decision-engine.md` | active (M3-T1 through M3-T9) | Bounded probabilistic judgment: the three question kinds with their versioned identity, the `DecisionEngine` port and its complete JSON-representable result, the harness-owned confidence derivation, per-question confidence bands that fail closed, the deterministic policy API a stored result replays through, the `DecisionRecord` that keeps the raw result and the policy outcome apart, the engine-free `replayDecisions()`, the `verify` primitive and its repair instructions, and the calibration metrics. |
